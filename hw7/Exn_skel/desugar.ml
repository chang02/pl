...
let removeExn : xexp -> xexp = fun xexp ->
	let rec cps : xexp -> xexp = fun e ->
		...
		| App (e1, e2) ->
			...
			let tmp2 = Fn (f, (apap (cps e2) tmp1 (Var h))) in
		...
		| If (e, e1, e2) ->
			...
			let tmp2 = (apap (cps e1) tmp1 (Var h)) in
			let t2 = new_var () in
			let tmp3 = Fn (t2, App (Var k, Var t2)) in
			let tmp4 = (apap (cps e2) tmp3 (Var h)) in
			let tmp5 = Fn (v1, If (Var v1, tmp2, tmp4)) in
			let tmp6 = (apap (cps e) tmp5 (Var h)) in
			Fn (k, Fn (h, tmp6))
		| Equal (e1, e2) ->
			let k, h = new_var (), new_var () in
